 ...
 const sortByRatingFunc = ()=>{
  ...
  const filterSearch = () => {
    const { method, rating } = filterState;
      ...
      filteredArr = filteredArr.sort((a, b) => sortByRating ? (b.rating - a.rating) : (a.rating - b.rating))
      setData(filteredArr) 
  }
...
<div class="mb-2">
  ...
  <div class="mb-2">
    <label >Rating</label>
    <input type="text" class="form-control form-control-sm" onChange={handleChange} value={rating} name='rating' />
  </div>
  <div class="mb-2">
  ...
  <div class="mb-2">
  Payment Method:{""}
  <select name="payment_methods" class="form-control form-control-sm"  onChange={handlePaymentMethodChange}>
  ...
  <div class="mb-2">
  <button type="button" class="btn btn-outline-primary btn-sm" onClick={handlesubmit}>ADD RESTAURANT</button>
...
<div className="stars">
  ...
  <button className={`${filterState.rating === 4 ? 'isActive' : 'isnotactive'}`} onClick={(e)=>{handlesorting(4)}}>4 Star</button>
  <button className={`${filterState.rating === 3 ? 'isActive' : 'isnotactive'}`} onClick={(e)=>{handlesorting(3)}}>3 Star</button>
  <button className={`${filterState.rating === 2 ? 'isActive' : 'isnotactive'}`} onClick={(e)=>{handlesorting(2)}}>2 Star</button>
  <button className={`${filterState.rating === 1 ? 'isnotactive' : 'isnotactive'}`} onClick={(e)=>{handlesorting(1)}}>1 Star</button>
  <button className={`${filterState.rating === '' ? 'isActive' : 'isnotactive'}`} onClick={(e)=>{handlesorting('')}}>All</button>
</div>
<div className="payment">
  <h5 className="sortrating">SORT BY PAYMENT</h5>
  <button className={`${filterState.method === 'card' ? 'isActive' : 'isnotactive'}`} onClick={()=>{handlepayment("card")}}>Card Payment</button>
  <button className={`${filterState.method === 'cash' ? 'isActive' : 'isnotactive'}`}  onClick={()=>{handlepayment("cash")}}>Cash Payment</button>
  <button className={`${filterState.method === 'upi' ? 'isActive' : 'isnotactive'}`}  onClick={()=>{handlepayment("upi")}}>UPI Payment</button>
  <button className={`${filterState.method === 'all' ? 'isActive' : 'isnotactive'}`}  onClick={()=>{handlepayment("all")}}>All</button>
</div>
<div className="sort">
<h5 className="sortrating">SORT</h5>
  <button className={`${sortByRating ? 'isActive' : 'isnotactive'}`} onClick={sortByRatingFunc}>{sortByRating ? "High to Low rating" : "Low to High rating"}</button>
...
<div>
  {data.map(({username,
    ...
    <div className="main">
    <div> <img src={image} alt="" height='32px' width='32px' /></div>
    <div>
